...
export async function GET(request: NextRequest) {
    const prisma = new PrismaClient(),
        r = await fetch("https://www.travelinedata.org.uk/noc/api/1.0/nocrecords.xml"),
        xml = await r.text(),
        json = await parseStringPromise(xml),
        lines = json.travelinedata.NOCLines[0].NOCLinesRecord.filter((line: any) => line.DateCeased[0] == "" && line.Duplicate[0] != "Dup");
